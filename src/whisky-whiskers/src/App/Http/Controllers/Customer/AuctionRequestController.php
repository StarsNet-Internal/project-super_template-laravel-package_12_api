...
class AuctionRequestController extends Controller
{
    ...
    public function createAuctionRequest(Request $request)
    {
        ...
        if (is_null($variant)) abort(404, 'Variant not found');

        // Get Product
        $product = $variant->product;
        $customer = $this->customer();
        if ($product->owned_by_customer_id != $customer->_d) abort(403, 'This product does not belong to the customer');
