...
class ProductManagementController extends Controller
{
    ...
    public function getRelatedAuctionProductsUrls(Request $request)
    {
        ...
        // Get configuraion
        $config = Configuration::where('slug', 'main-store-category-ordering')->latest()->first();
        ...
        $originalProductCategories = $originalProduct->categories->pluck('id')->values()->all();

        foreach ($parentCategoryIDs as $key => $parentCategoryID) {
            $childrenCategoryIDs = Category::where('parent_id', $parentCategoryID)->pluck('id')->all();
        ...
        */
        return collect($products)
            ->pluck('id')
            ->chunk($itemsPerPage)
            ->map(fn($chunk) => route('whiskywhiskers.products.ids', [
    ...
    public function getAuctionProductsByIDs(Request $request)
    {
        ...
        $products = $this->getProductsInfoByAggregation($request->ids);
    ...
    private function getProductsInfoByAggregation(array $productIDs, ?Store $store = null)
    {
        ...
        // Extract variables
        $storeID = $store->_id;
        $warehouseIDs = $store->warehouses()->statusActive()->get()->pluck('id')->all();
